...
class CexAPI {
    ...
    async rstap(authData, multiTapsEnergy) {
        ...
        try {
            ...
            this.log(`Lỗi Reset Tap: ${error.message}`);
        }
    }

    async claimMultiTaps(authData) {
        const url = "https://cexp.cex.io/api/v2/claimMultiTaps";
        
        let totalClaimed = 0;
        let userData = await this.getUserInfo(authData);
    ...
    async buyUpgrade(authData, upgradeData) {
    ...
    async processUpgrades(authData, balanceUSD) {
        try {
            const gameConfig = await this.getGameConfig(authData);
            const userCards = await this.getUserCards(authData);
            ...
            for (const category of gameConfig.upgradeCardsConfig) {
                for (const upgrade of category.upgrades) {
                    ...
                    if (currentLevel < upgrade.levels.length) {
                        ...
                        // Kiểm tra nếu số dư USD nhỏ hơn chi phí nâng cấp
                        if (ccy === "USD" && balanceUSD < cost) {
                            this.log(`Bỏ qua thẻ ${upgrade.upgradeName}: không đủ USD (cần ${cost} USD, có ${balanceUSD} USD)`, 'warning');
                            continue;
                        }
                        ...
                        try {
                            const result = await this.buyUpgrade(authData, upgradeData);
                            if (result.status === "ok") {
    ...
    async convertCrypto(authData, userData) {
        const url = "https://cexp.cex.io/api/v2/convert";
        const lastPrice = await this.getConvertData(authData);
    ...
    async main() {
        ...
        while (true) {
            for (let i = 0; i < data.length; i++) {
                try {
                    const result = await this.getUserInfo(data[i]);
                    if (result.status === "ok") {
                        const userData = result.data;
                        console.log(`========== Tài khoản ${i + 1} | ${userData.first_name.green} ==========`);
                        const claimCryptoResult = await this.claimCrypto(data[i]);
                        ...
                        if (hoiswap) {
                            if (userData.balance_BTC > 0 && swapPercentage > 0) {
                                try {
                                    ...
                                    const newBalanceUSD = await this.convertCrypto(data[i], userData, btcToConvert);
                                    userData.balance_USD = newBalanceUSD;
                                    this.log(`Balance USD mới sau khi swap: ${newBalanceUSD}`, 'success');
                                } catch (error) {
                        ...
                        if (userData.multiTapsEnergy > 100) {
                            try {
                                const claimMultiTapsResult = await this.claimMultiTaps(data[i]);
                                this.log(claimMultiTapsResult.message, 'success');

                                const finalUserInfo = await this.getUserInfo(data[i]);
...
//                                this.log(`Không thể tap: ${error.message}`, 'error');
                        ...
                        if (buyCardsDecision) {
                            await this.processUpgrades(data[i], userData.balance_USD);
                        }
