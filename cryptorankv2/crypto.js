    ...
    async processTasks(token) {
        try {
            ...
            for (const task of tasks) {
                const { id, name, lastClaim, cooldown, isRepeatable } = task;
                ...
                if (isRepeatable) {
                    if (lastClaim === null) {
                        ...
                        canClaim = currentTime >= lastClaim + cooldown * 1000;
                    }
                } else {
                    canClaim = lastClaim === null;
                }
    
                if (canClaim) {
                    try {
                        await this.claimTask(token, id);
                        this.log(`${'Làm nhệm vụ'.yellow} ${name} ${'thành công!'.green}`);
                    } catch (claimError) {
                        this.log(`${'Làm nhệm vụ'.yellow} ${name} ${'thất bại!'.red}`);
                    }
                } else if (isRepeatable && lastClaim !== null) {
                    const remainingTime = (lastClaim + cooldown * 1000 - currentTime) / 1000;
                    this.log(`${name}: ${'Còn'.yellow} ${Math.ceil(remainingTime)} ${'giây để có thể claim lại'.yellow}`);
                }
            }
        } catch (error) {
