...
class GLaDOS {
    constructor() {
        ...
        this.durovUrl = 'https://major.glados.app/api/durov/';
    ...
    async getDurovPayload() {
        try {
            ...
            this.log(`Error fetching Durov payload: ${error.message}`.red);
            return null;
        }
    }
    ...
    async handleDurovTask(token) {
        try {
            const getResponse = await axios.get(this.durovUrl, { headers: this.headers(token) });
            ...
            if (today.hasSame(payloadDate, 'day')) {
                ...
                const postResponse = await axios.post(this.durovUrl, payload, { headers: this.headers(token) });
            ...
            if (error.response && error.response.status === 400 && error.response.data.detail) {
                ...
                if (detail.blocked_until) {
                    const blockedTime = DateTime.fromSeconds(detail.blocked_until).setZone('system').toLocaleString(DateTime.DATETIME_MED);
                    this.log(`Tìm câu đố Durov không thành công, cần mời thêm ${detail.need_invites} bạn hoặc chờ đến ${blockedTime}`.yellow);
                } else {
                    this.log(`Error in Durov task: ${detail}`.red);
                }
            } else {
